...
# -------------------------- then, system will perform query --------------------------------#
            try:
                ...
                return csp_node, collect( question_node) as q_nodes, collect( distinct  entity_value) as entity_values
    ...
    # -------------------------- then, form dispatchd massage ---------------------------------#
                ...
                if len(result) == 0:
                    ...
                    try:
                        result[0].data()['entity_values'][0]['description']
                        msg_entity_value = ' \n' +  result[0].data()['entity_values'][0]['name'] + ':' +  result[0].data()['entity_values'][0]['description'] 
                    except:
                        msg_entity_value = ''

                    question_list = [ item for item in result[0].data()['q_nodes'] if 'label' in item.keys() and item['label'] == 'question']
