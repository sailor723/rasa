...
#----------------------------check question_list ---------------------------------------------------------#
            ...
            try:
                ...
                if question_list and sub_list[0] in question_list:              # check question_list, also sub[0] in question_list
                    ...
                    msg_QA = 'DL04问题回答' + '\n'
                    msg = msg_QA + result[0].data()['answer_node.name'] 
...
# -------------------------- then, system will perform query --------------------------------#
            try:
                ...
                return collect(distinct csp_node) as csp_nodes, collect( distinct question_node) as q_nodes, collect( distinct  entity_value) as entity_values
                ORDER BY csp_nodes
                """
                result = conn.query(query, parameters=params)
