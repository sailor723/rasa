...
class ActionCheckProtocol(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        site_info = df_user[df_user.user_id == 'Andy']['site_id']
...
# -------------------------- then, system will perform query --------------------------------#
            try:
                ...
                params = {'node_sub': sub_list, 'version': version_number}
                
                query = """
                match (question_node) <-[to_questions*0..1] - (answer_node) <- [has_answer*0..1] -(csp_node) -[r*] ->(entity_node) -[*0..3] ->(entity_value)
                where csp_node.label in ['入组标准', '排除标准','DL04'] and ( entity_node.name in $node_sub ) and csp_node.version = $version
    ...
    # -------------------------- then, form dispatchd massage ---------------------------------#
                ...
                dispatcher.utter_message(text=(csp_version + page_num +  msg + msg1 + ' \n' +  msg2), buttons= button_list)
