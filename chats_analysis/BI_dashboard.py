...
#-------------------------------- convert str to dict -----------------------------------------------#
new_col = [eval(item) if item != None else item for item  in df.parse_data]    # if None
df.parse_data = new_col
new_col = [eval(item) if item != None else item for item  in df.data]          # if None
df.data = new_col
...
df_final['sub_extractor'] = [ 'Other' if x == 0 else x for x in df_final['sub_extractor']]
...
#------------------------------Streamlit -----------------------------------------------#
with header:
    st.title("DL04  Project")

with dataset:
    st.title("Conversational Dataset Analysis")
...
#---------------------------Question distribution -----------------------------------
    ...
    Question_distribution = pd.DataFrame(df_final['text'].value_counts()).head(20)
...
#--------------------------- for Q&A log -------------------------------------------------@
    ...
    entity_question_df = pd.DataFrame(df_final[df_final['clean_text'].isin(question_button_list)]['clean_text'].value_counts()).head(20)
